...
```{r}
lm.formula <- "log(pm2.5)~year + month + day + DEWP + TEMP + PRES + Iws + Is^2 + Ir^2"
lm2 <- lm(as.formula(lm.formula), data = new.Beijing.data, interval = "confidence") # take the natural logarithm to PM2.5 data
par(mfrow=c(2,2))
plot(lm2)
```
```{r}
...
```{R}
# AIC BIC- backwards
lm.AIC <- step(lm2,direction="backward", data = new.Beijing.data)
lm.BIC <- step(lm2,direction="backward", data = new.Beijing.data, k=log(1826))

#forward selection
lm.forward <- lm(pm2.5~.,data = new.Beijing.data)
```
