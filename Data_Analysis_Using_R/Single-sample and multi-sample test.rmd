...
例子：反映 $11$ 个女性的每日摄入能量（千焦）记录（Altman，1991）。搜线，这些值在一个数据向量：
```{r}
...
```
也许你想检验一下这些女性的摄入能量是否与推荐值 $7725$ 千焦相差甚远。假设数据来自于正太分布，那么我们的母的就是检验这个分布是否满足$\mu = 7725$。 我们可以如下通过`t.test`进行这个检验：
```{r}
t.test(daily.intake, mu = 7725)
```
这里的输出结果和很多其他的标准假设检验是类似的，下面是对其的详细解说：

`One Sample t-test`

这是对我们所做的检验的描述。 `t.test`会自动发现用户需要的是一个单样本的检验

`data:data.intake`

这个是将要被检验的数据。这个信息是显而易见的，但是如果将代码与输出分离的话这就不一定了。这是可能发生的，例如，使用 `source` 函数去调用另一个文件中的代码。
